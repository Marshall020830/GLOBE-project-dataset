...
for root in trace:
    ...
    output_file = root + "/" + root + "_random_GPTo1mini.csv"
    ...
    for count, i in enumerate(pending_indices, start=1):
        ...
        data = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": f"I'm going to ask you a question. Please just make a simple choice—no extra explanation is needed. If you chose the previous answer, please output A, otherwise output B. Just answer A or B. Question: {question}"
                }
            ]
        }
        try:
